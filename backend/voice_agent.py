...
# ── Cartesia Russian voice IDs ───────────────────────────────────────────────
# Verified against Cartesia voice library /voices?language=ru.
# For Kazakh (kk) we reuse Russian voices — phonetically close enough,
# Cartesia has no native Kazakh voices.
MALE_VOICES = [
...
def pick_voice(case_id: int, gender: str, override: str | None = None) -> str:
    """Deterministically choose a voice id for a patient.

    Same case_id + gender → same voice forever, so the patient sounds
    consistent across sessions. Different patients of the same gender →
    different voices (within the pool).
    """
    if override:
        return override
    pool = FEMALE_VOICES if (gender or "").lower() == "female" else MALE_VOICES
    h = int(hashlib.sha1(f"{case_id}".encode()).hexdigest(), 16)
    return pool[h % len(pool)]


DEFAULT_INSTRUCTIONS = (
    "Ты — пациент на симуляционном приёме. Отвечай коротко, "
    "как обычный человек. Не называй диагноз сам, не давай "
    "медицинских советов. Отвечай на русском языке."
)
...
async def entrypoint(ctx: JobContext):
    ...
    voice_id = pick_voice(case_id, gender, voice_override)
    ...
    # the API endpoint already rejects kk requests, but we hard-code Russian
    # here too for safety in case the worker is invoked some other way.
